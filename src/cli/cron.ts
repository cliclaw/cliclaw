...
function computeSleep(baseSleep: number, stallCycles: number): number {
  if (stallCycles <= 0) return baseSleep;
  const multiplier = Math.min(Math.pow(1.5, stallCycles), 10);
  return Math.round(baseSleep * multiplier);
}
...
export async function cronCommand(args: string[]): Promise<void> {
  ...
  for (let cycle = 1; cycle <= config.maxLoop; cycle++) {
    ...
    if (cycle % 4 === 1) writeHeartbeat(config, cycle, consecutiveFails);

    saveSnapshot(paths.snapshotsDir, paths.stateFile, cycle);
    runPreCycle(config.hooks, config.projectRoot, cycle);
    ...
    if (success) {
      ...
      if (consecutiveFails >= 3 && !config.parallel && config.engines.length > 1) {
        ...
        if (isEngineAvailable(next.engine)) {
          ...
          appendToMemory(paths.memoryFile, `Engine rotation: ${currentAlias} failed ${consecutiveFails}x, switched to ${nextAlias}.`);
          activeIdx = nextIdx;
          consecutiveFails = 0;
        }
      }

      runOnFailure(config.hooks, config.projectRoot, cycle);
      ...
      if (stallCycles >= 10) {
        ...
        appendToMemory(paths.memoryFile, `Stall warning: ${stallCycles} cycles with no progress.`);
        sendNotification("CLIClaw stall", `${stallCycles} cycles with no progress`);
      }

      const sleepTime = computeSleep(config.sleepAfterFailure, stallCycles);
  ...
  writeHeartbeat(config, config.maxLoop, consecutiveFails);
  releaseLock(paths.lockDir);
