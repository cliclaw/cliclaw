...
- `src/agents/` — Agent registry and process runner
...
- **Config uses `agents` array** — no top-level `agent`/`model`. The first entry in `agents` is the primary.
...
1. Add the agent name to `AgentName` in `src/core/types.ts`.
2. Add default model to `DEFAULT_MODELS` in `src/core/config.ts`.
3. Add the agent name to `ALL_ENGINES` in `src/core/config.ts`.
4. Add the agent config to `src/agents/registry.ts` (command, buildArgs, model, flags).
5. Add the CLI command name to `ENGINE_COMMANDS` in `src/cli/setup.ts`.
6. Add pricing to `PRICING` in `src/core/cost.ts`.
7. Add tests in `tests/agents/registry.test.ts`.
